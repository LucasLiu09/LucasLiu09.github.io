...
- 系统会先收集**该模型 + 该操作**下适用于当前用户的规则：
  ...
  - **组规则**：`groups` 与用户组有交集
- 然后合并成最终域（关键点在于：**全局规则 AND，组规则 OR**）：
...
2. **记录级检查（`ir.rule`）**  
   - 若该操作涉及具体记录集合，则必须满足**记录规则最终合并后的 domain**（全局规则 AND + 组规则 OR）  
   - 否则拒绝（`AccessError`）
3. **业务逻辑层面的额外限制**  
