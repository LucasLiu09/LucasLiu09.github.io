---
title: ListView扩展
description: ListView扩展
sidebar_label: ListView扩展
keyword:
    - odoo
    - odoo development
tags: [odoo]
...
## 在`<Field/>`标签设置标题行class

通过在`<field/>`标签内设置`options`属性对应`thClassNames`为`key`的值，向`list`表格的标题行添加`class`。
...
:::tip[💡单击跳转视图]
单击跳转视图可以查阅[文档](/odoo/dev_notes/source_code_analysis/views/list_view_open_record_dynamic.mdx)
:::

> 以下为`js_class`方式实现，可根据需要修改为`patch`。
