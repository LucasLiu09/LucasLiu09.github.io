...
def serialize_datetime(input_datetime, source_tz_name=""):
    """
    将输入的datetime对象转换为UTC时区下的datetime对象
    Args:
        input_datetime: 输入的datetime对象
        source_tz_name: 源时区名称
    Returns:
        datetime对象
    """
    if not source_tz_name:
        source_tz_name = "UTC"
    utc_datetime = convert_datetime_timezone(input_datetime, source_tz_name, "UTC", rm_tzinfo=True)
    return utc_datetime


def convert_datetime_timezone(input_datetime, source_tz_name="UTC", target_tz_name="UTC", rm_tzinfo=False):
    """
    将输入的datetime对象转换为目标时区下的datetime对象
    Args:
        input_datetime: 输入的datetime对象
        source_tz_name: 源时区名称
        target_tz_name: 目标时区名称
        rm_tzinfo: 返回结果是否移除时区信息
    Returns:
        datetime对象
    """
    # 仅处理 datetime 对象；空值或其他类型保持原样返回。
    if not input_datetime or not isinstance(input_datetime, datetime.datetime):
        return input_datetime
    
    try:
        source_tz = pytz.timezone(source_tz_name or "UTC")
        target_tz = pytz.timezone(target_tz_name or "UTC")
    except pytz.UnknownTimeZoneError as e:
        raise     # UserError(_("无效的时区名称: %s") % e)

    try:
        # 将传入的 datetime 视为源时区的本地时间。
        if input_datetime.tzinfo:
            source_datetime = input_datetime.astimezone(source_tz)
        else:
            source_datetime = source_tz.localize(input_datetime, is_dst=False)  # UTC = no DST
        # 转换为目标时区的datetime
        target_datetime = source_datetime.astimezone(target_tz)
        return target_datetime.replace(tzinfo=None) if rm_tzinfo else target_datetime
    except Exception as e:
        raise     # UserError(_("时区转换失败: %s") % e)

def get_today_by_timezone(tz_name="UTC", timestamp=None):
    """
    获取指定时区下的当天日期。
    Args:
        tz_name: 目标时区名称
        timestamp: 可选的基准 datetime，默认为当前 UTC 时间; 该时间点需要是UTC时间。
    Returns:
        date对象
    """
    today = timestamp or datetime.datetime.now(datetime.timezone.utc).replace(tzinfo=None)
    try:
        target_tz = pytz.timezone(tz_name or "UTC")
        if today.tzinfo:
            today_utc = today.astimezone(pytz.UTC)
        else:
            today_utc = pytz.UTC.localize(today, is_dst=False)
        return today_utc.astimezone(target_tz).date()
    except Exception:
        # _logger.debug("failed to compute timezone-specific today date, using UTC value", exc_info=True)
        return today.date()

# 使用示例: 将指定时区的早上八点钟转换为utc时区的naive datetime
def test_timezone(self):
    tz_name = "Asia/Hong_Kong"
    tz = pytz.timezone(tz_name)
    today = get_today_by_timezone(tz_name)
    # 指定时区当地时间的早上8点
    local_m8 = tz.localize(datetime.datetime.combine(today, datetime.time(8,0,0)))
    utc_time = utils.serialize_datetime(input_datetime=local_m8, source_tz_name=tz_name)
    raise ValidationError(f"localtime: {local_m8}, utc_time: {utc_time}")
...
class ExcelWriter:
    ...
    def __init__(self, bytes_data: BytesIO = None,
        ...
        # TODO: 设置默认样式( xlsxwriter.Workbook(filename, options), 通过options传入default_format_properties )
...
class SimpleMatrix:
    ...
    def __init__(self):
        ...
        self.rows = defaultdict(dict)  # row_key -> {col_key: value}
        self.cols = defaultdict(dict)  # col_key -> {row_key: value}
        self.row_keys = []  # 维护插入顺序
        self.col_keys = []
        self._row_set = set()
        self._col_set = set()
    ...
    def set(self, row_key, col_key, value):
        """设置数据"""
        # 更新索引
        if row_key not in self._row_set:
            self.row_keys.append(row_key)
            self._row_set.add(row_key)
        if col_key not in self._col_set:
            self.col_keys.append(col_key)
            self._col_set.add(col_key)

        # 存储数据到三个地方
        self.data[(row_key, col_key)] = value
        self.rows[row_key][col_key] = value
        self.cols[col_key][row_key] = value

    def get(self, row_key=None, col_key=None, default=None):
        """
        灵活的获取接口
        - get(row, col): 获取单个值
        - get(row=row): 获取整行
    ...
    def delete(self, row_key=None, col_key=None):
        """删除数据"""
        if row_key is not None and col_key is not None:
            # 删除单个单元格
            if (row_key, col_key) in self.data:
                del self.data[(row_key, col_key)]
                del self.rows[row_key][col_key]
                del self.cols[col_key][row_key]
        elif row_key is not None:
            # 删除整行
            for col_key in list(self.rows[row_key].keys()):
                self.delete(row_key, col_key)
            if row_key in self._row_set:
                self.row_keys.remove(row_key)
                self._row_set.remove(row_key)
        elif col_key is not None:
            # 删除整列
            for row_key in list(self.cols[col_key].keys()):
                self.delete(row_key, col_key)
            if col_key in self._col_set:
                self.col_keys.remove(col_key)
                self._col_set.remove(col_key)
    ...
    def to_list(self):
        ...
        for row_key in self.row_keys:
            ...
            for col_key in self.col_keys:
                row.append(self.data.get((row_key, col_key), None))
            result.append(row)
        return result

    def __repr__(self):
        return f"SimpleMatrix(rows={len(self.row_keys)}, cols={len(self.col_keys)}, cells={len(self.data)})"
```
