...
def convert_datetime_timezone(input_datetime, source_tz_name="UTC", target_tz_name="UTC", rm_tzinfo=False):
    ...
    if input_datetime.tzinfo:   # 如果输入的datetime对象有时区信息，则先转换为naive datetime对象
    ...
    # 返回目标时区下的 datetime，便于继续按 Odoo Datetime 习惯处理。
    if rm_tzinfo:
        return source_datetime.astimezone(target_tz).replace(tzinfo=None)
    else:
