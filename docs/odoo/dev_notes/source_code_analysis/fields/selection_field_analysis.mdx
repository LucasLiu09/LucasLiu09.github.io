...
## 1. SelectionField 是什么：一个“用 `<select>` 渲染 selection 或 many2one”的字段组件
...
## 2. 组件 props 来源：`Field` 包装器如何把 record/name/value/update 注入进来
...
## 3. SelectionField 的核心逻辑（JS）：options / string / value / onChange

### 3.1 options：分支处理 selection vs many2one
...
### 3.3 value：用于 `<select>` 比对 selected 的“原子值”
...
### 3.4 stringify/parse：用 JSON 承载 `<option value>`
...
### 3.5 onChange：把 `<select>` 的原子值写回 record
...
## 4. 模板结构（XML）：readonly 与 editable 两种渲染
...
## 5. many2one 作为“下拉 selection”的关键：preloadedData 预加载机制

### 5.1 新模型路径：`views/relational_model.js` 的 `loadPreloadedData()`
...
### 5.2 SelectionField 的预加载注册：`registry.category("preloadedData").add("selection", ...)`
...
## 6. 兼容/遗留路径：`legacySpecialData` 如何把 legacy specialData 变成 record.preloadedData
...
### 6.1 OWL → legacy fieldsInfo：`views/legacy_utils.js`
...
### 6.2 legacy BasicModel：`_fetchSpecialRelation` 实际就是 `name_search`
...
## 7. registry 与 displayName：为什么这么写
