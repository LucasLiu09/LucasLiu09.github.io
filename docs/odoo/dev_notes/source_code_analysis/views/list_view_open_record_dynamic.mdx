---
title: List View - 动态指定打开界面
description: List View - 动态指定打开界面
sidebar_label: List View - 动态指定打开界面
keyword:
    - odoo
    - odoo development
tags: [odoo]
...
：🦾我想要在列表页面点击一行数据时，属于这个类型打开这个界面，那个类型打开另一个界面。

：嗯。。。行

:::

总结用户需求就是希望点击列表界面的一行数据时，根据数据的差异打开不同的Form界面。

## 解析与实现

下面我们基于Odoo16版本的代码来做一个解析(解析过程中均为片段，完整源码在底部列出)：

那么我们从最外层往回推，最外层是`List View`，负责渲染列表的是`ListRenderer`。

从`ListRenderer`中不难发现，行数据属于template: `web.ListRenderer.RecordRow`，进而找到行内元素`column.type='field'`的点击事件：`t-on-click="(ev) => this.onCellClicked(record, column, ev)"`
...
那么进一步来到`list_renderer.js`，查看`ListRenderer.onCellClicked()`，从这个function可以看到打开界面的逻辑由**`this.props.openRecord(record)`**负责处理

`ListRenderer`在这里的父组件是`ListController`，查看`List_controller.xml`文件，来确认`this.props.openRecord`绑定的是父组件的哪一个函数。

以下代码摘自`list_controller.xml`，找到`<t t-component="props.Renderer"/>`，此处引用的`component`由`ListController.props.Renderer`指定，指定的位置位于`list_view.js` -> `const listView`.(原生代码指定的便是`ListRenderer`)

```xml title="list_controller.xml 片段"
...
从这里可以看到`ListRenderer.props.openRecord`绑定的是`ListController.openRecord()`.

```javascript title="list_controller.js 片段"
...
本章节的关键就在于这个`openRecord()`函数，我们可以看到当`this.archInfo.openAction`成立时，将会调用`this.actionService.doActionButton()`进行跳转。

`doActionButton`: 在当前操作之上执行`doAction`操作（通常用于单击视图中的按钮），type可以是`object`、`action`，当`type="object"`时，将会调用给定模型的给定方法，这个方法将会返回一个action用于后续跳转。

先说这里可以通过`doActionButton`能做到什么事情:

1. 指定`type="action"`：直接跳转到action指定的界面。传入的action是字符串，在xml中编写的ir.actions.act_window的id。
2. 指定`type="object"`：调用给定模型的方法，可以在方法中自定义逻辑来指定跳转的action，从而实现动态打开界面。

**本章节的需求主要是通过第2种方式来实现。**

:::note
对于`doActionButton`的详细说明，请查看文档(暂未编写)，可自行查阅源码addons/web/static/src/webclient/actions/action_service.js.
:::


再过头来看`archInfo.openAction`是如何指定的，View的archInfo通常由`<ViewType>ArchParser`(extends XMLParser)解析得到

那么查看`list_arch_parser.js`文件，找到`ListArchParser.parse()`方法，查阅该方法，可以发现`openAction`属性是包含在tree/list标签内设置。

至此，我们知道了需要实现本章节需要的所有内容。

实现步骤：
1. 在tree view上设置`action`和`type`属性，`type="object" action="open_record_dynamic_func_name"`。
2. 编写`open_record_dynamic_func_name`的逻辑，根据需求编写自定义逻辑，最终返回一个action，返回的action最好是由`self.env['ir.actions.act_window']._for_xml_id('xml_id')`得到的字典进行转换。
