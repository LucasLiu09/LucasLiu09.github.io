...
### 通过_auto_init新建索引(index)

使用示例: 以下在hh_leave表上新增一条索引。此处用到odoo.tools.sql.craete_index(可以通过`from odoo.tools import create_index`引用)。
...
:::note[tips]
官方**更推荐将创建索引的操作放在`init`函数中**。因为`init`函数会在`_auto_init`之后被调用。
