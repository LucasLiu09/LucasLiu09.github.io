...
WHERE imd.module = '<module>'
  AND imd.name = '<group_xml_id>'
  AND imd.model = 'res.groups'
  and u.active = true
ORDER BY p.name, u.login;
...
JOIN res_users u ON u.id = rel.uid
JOIN res_partner p ON p.id = u.partner_id
WHERE g.name->>'en_US' = '<group name>'	-- zh_CN
  AND u.active = true
ORDER BY p.name, u.login;
```
...
target_group AS (
    ...
    FROM ir_model_data
    WHERE module = '<module>'
      AND name = '<target_group_xml_id>'
      AND model = 'res.groups'
)
INSERT INTO res_groups_users_rel (gid, uid)
SELECT target_group.gid, rel.uid
FROM res_groups_users_rel rel
JOIN source_group ON rel.gid = source_group.gid
CROSS JOIN target_group
WHERE NOT EXISTS (
    SELECT 1
    FROM res_groups_users_rel existing
    WHERE existing.gid = target_group.gid
      AND existing.uid = rel.uid
)
RETURNING uid;
```
